#!/usr/bin/env python3
"""
Realistic Trading Simulator for Zone Fade Strategy

This module simulates realistic trading with proper account balance tracking,
position sizing, slippage, and commissions to calculate accurate drawdowns.
"""
...
class RealisticTradingSimulator:
    """Realistic trading simulation with proper account balance tracking."""
    
    def __init__(self, 
                 initial_balance: float = 10000.0,
                 max_equity_per_trade: float = 0.10,  # 10% max equity per trade
                 slippage_pct: float = 0.02,  # 2% slippage
                 commission_per_trade: float = 5.0):  # $5 commission per trade
        """
    ...
    def simulate_trading_sequence(self, entry_points: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            DataFrame with all trades and balance tracking
        """
        trades = []
    ...
    def calculate_drawdown_metrics(self, trades: pd.DataFrame) -> Dict[str, float]:
        ...
        # Calculate recovery time (time to reach new high after drawdown)
        recovery_times = []
        for i, row in trades_sorted.iterrows():
            if row['drawdown'] < 0:
                # Find when balance recovers to previous high
                future_balances = trades_sorted.loc[i:]['running_balance']
                recovery_idx = (future_balances >= row['running_max']).idxmax()
                if recovery_idx and recovery_idx != i:
    ...
    def print_trading_summary(self, report: Dict[str, any]):
        ...
        print("\n" + "="*100)
        print("REALISTIC ZONE FADE TRADING SIMULATION")
        print("="*100)
        ...
        print(f"   Slippage: {summary['slippage_pct']*100:.1f}%")
        ...
        print(f"   Avg Recovery Time: {perf['avg_recovery_time']:.1f} trades")
...
def simulate_realistic_trading(csv_file_path: str, 
                             initial_balance: float = 10000.0,
                             max_equity_per_trade: float = 0.10,
                             slippage_pct: float = 0.02,
                             commission_per_trade: float = 5.0) -> Dict[str, any]:
    """
    Simulate realistic trading with proper account balance tracking.
    ...
    print("🔄 Simulating realistic trading sequence with account balance tracking...")
...
if __name__ == "__main__":
    ...
    report = simulate_realistic_trading(csv_file)