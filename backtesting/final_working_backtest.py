#!/usr/bin/env python3
"""
Final Working Backtest

This version fixes the zone touch history bug and properly implements the framework.
"""
...
def create_adequate_bars_data(entry_points: pd.DataFrame) -> Dict[str, List]:
    """Create adequate bars data that covers all bar indices."""
    bars_data = {}
    
    for symbol in entry_points['symbol'].unique():
        symbol_entries = entry_points[entry_points['symbol'] == symbol].sort_values('bar_index')
        
        # Get the maximum bar index for this symbol
        max_bar_index = symbol_entries['bar_index'].max()
        
        # Create bars that cover all indices with buffer
        bars = []
        base_price = 100.0
        
        # Create enough bars to cover all bar indices
        total_bars = max_bar_index + 100  # Add buffer
...
def run_final_working_backtest():
    """Run the final working backtest with corrected zone touch logic."""
    
    print("🚀 FINAL WORKING FRAMEWORK BACKTEST")
    print("=" * 70)
    ...
    print("📊 Creating adequate bars data...")
    bars_data = create_adequate_bars_data(entry_points)
    ...
    for _, entry in entry_points.iterrows():
        ...
        if bar_index >= len(bars):
            print(f"   ⚠️  Skipping {entry['entry_id']} - bar_index {bar_index} >= bars length {len(bars)}")
            continue
        
        # CRITICAL FIX: Reset zone touch history for each entry to simulate fresh trading session
