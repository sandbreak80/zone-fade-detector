#!/usr/bin/env python3
"""
Final Working Plotly Zone Fade Entry Point Visualizer

A final version that completely avoids timestamp arithmetic issues.
"""
...
class FinalPlotlyVisualizer:
    """Final working visualization using Plotly."""
    ...
    def create_final_chart(self, df: pd.DataFrame, entry_idx: int, 
                          symbol: str, entry_id: str, qrs_score: float) -> go.Figure:
        """Create a final working Plotly candlestick chart."""
    ...
    def create_visualizations(self, data: Dict[str, List[OHLCVBar]], 
        ...
        """Create final visualizations for entry points."""
        print("🎨 Creating final Plotly visualizations...")
        
        # Create output directory
        plotly_dir = output_dir / "final_plotly_charts"
        ...
        for i, entry in enumerate(entry_points[:3]):  # Limit to first 3 for testing
            try:
                ...
                # Find entry point using string matching to avoid timestamp arithmetic
                entry_idx = None
                entry_timestamp_str_clean = entry_timestamp_str.replace('+00:00', '').replace('Z', '')
                
                for idx, row in df.iterrows():
                    row_timestamp_str = str(row['timestamp']).replace('+00:00', '').replace('Z', '')
                    if entry_timestamp_str_clean in row_timestamp_str or row_timestamp_str in entry_timestamp_str_clean:
                        entry_idx = idx
                        break
                ...
                # Create Plotly chart
                fig = self.create_final_chart(
                    window_df, new_entry_idx, symbol, entry_id, qrs_score
                )
                
                # Save as HTML
                entry_timestamp = pd.to_datetime(entry_timestamp_str)
                filename = f"{symbol}_{entry_timestamp.strftime('%Y%m%d_%H%M')}_final_chart.html"
        ...
        print(f"📊 Final visualization results:")
    ...
    def run_visualization(self, data_dir: Path, output_dir: Path) -> bool:
        """Run the final visualization process."""
        print("🚀 Final Plotly Zone Fade Visualizer")
        print("=" * 50)
        ...
        if success:
            ...
            print("❌ Final visualization failed!")
...
def main():
    ...
    visualizer = FinalPlotlyVisualizer()
