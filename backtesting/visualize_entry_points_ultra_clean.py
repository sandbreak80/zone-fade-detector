#!/usr/bin/env python3
"""
Ultra Clean Zone Fade Entry Points Visualization Tool

This script generates ultra clean visualizations with:
- NO filled rectangles or background areas
- NO overlapping label windows
- Clean lines only for all elements
...
def create_ultra_clean_visualization(
    entry_point: pd.Series,
    bars: List[OHLCVBar],
    symbol: str,
    output_dir: Path
) -> bool:
    """Create an ultra clean visualization with NO filled rectangles."""
    ...
    # Create the figure with ultra clean layout
    fig, (ax_price, ax_volume) = plt.subplots(2, 1, figsize=(16, 10), 
                                             gridspec_kw={'height_ratios': [3, 1], 'hspace': 0.15})
    
    # Set ultra clean styling
    plt.style.use('default')
    
    # === ULTRA CLEAN PRICE CHART - NO FILLED RECTANGLES ===
    
    # Plot candlesticks with ultra clean styling
    for i, (timestamp, open_price, high, low, close) in enumerate(zip(timestamps, opens, highs, lows, closes)):
        # Ultra clean candlestick colors
        is_green = close >= open_price
        color = '#2e7d32' if is_green else '#c62828'  # Clean green/red
        edge_color = '#1b5e20' if is_green else '#b71c1c'  # Darker edges
        
        # Body - NO FILL, just outline
        body_height = abs(close - open_price)
        body_bottom = min(open_price, close)
        if body_height > 0:
            # Draw body as outline only, no fill
            ax_price.plot([timestamp, timestamp], [body_bottom, body_bottom + body_height], 
        ...
        # Wicks - clean lines only
        ax_price.plot([timestamp, timestamp], [low, high], color='black', linewidth=1)
    ...
    # Plot VWAP with clean line
    if vwap_values:
        ax_price.plot(timestamps, vwap_values, color='#7b1fa2', linewidth=2.5, 
                     label='VWAP', alpha=0.9, linestyle='-')
    
    # Plot zone level with clean line
    ax_price.axhline(y=zone_level, color='#f57c00', linestyle='--', linewidth=2.5, 
                    label=f'Zone Level: ${zone_level:.2f}', alpha=0.9)
    
    # Highlight entry point with clean markers
    entry_timestamp_vis = timestamps[vis_entry_idx]
    ax_price.axvline(x=entry_timestamp_vis, color='#d32f2f', linestyle='-', linewidth=2, 
                    alpha=0.8, label='Entry Time')
    
    # Mark entry point with clean marker
    ax_price.scatter(entry_timestamp_vis, entry_price, color='#d32f2f', s=200, 
                    marker='*', edgecolor='white', linewidth=2, zorder=10, label='Entry Price')
    
    # Add entry window with clean vertical lines (NO FILL)
    window_end_time = entry_timestamp_vis + timedelta(minutes=window_duration)
    ax_price.axvline(x=window_end_time, color='#ffa000', linestyle=':', linewidth=2, 
    ...
    ax_price.legend(loc='upper left', fontsize=10, framealpha=0.95, fancybox=True, shadow=True)
    ...
    # === ULTRA CLEAN VOLUME CHART - NO FILLED RECTANGLES ===
    
    # Calculate volume colors based on price direction
    volume_colors = []
    for i, (close, open_price) in enumerate(zip(closes, opens)):
        if close >= open_price:
            volume_colors.append('#2e7d32')  # Clean green
        else:
            volume_colors.append('#c62828')  # Clean red
    
    # Plot volume bars with clean styling - NO FILL
    for i, (timestamp, volume, color) in enumerate(zip(timestamps, volumes, volume_colors)):
        # Draw volume as vertical line, not filled bar
        ax_volume.plot([timestamp, timestamp], [0, volume], color=color, linewidth=3, alpha=0.7)
    
    # Highlight entry point volume with clean marker
    ax_volume.plot([timestamps[vis_entry_idx], timestamps[vis_entry_idx]], 
                  [0, volumes[vis_entry_idx]], color='#d32f2f', linewidth=4, alpha=1.0)
    
    # Add volume spike indicator if applicable - clean line only
    if entry_point.get('volume_spike', False):
        avg_volume = np.mean(volumes)
        spike_threshold = avg_volume * 1.8
        ax_volume.axhline(y=spike_threshold, color='#f57c00', linestyle='--', 
    ...
    # Add clean info text - positioned to avoid overlap
...
Entry: {volumes[vis_entry_idx]:,.0f}"""
    ...
    # Add info box to the price chart - positioned to avoid overlap
    ...
    # Add quality indicator - positioned to avoid overlap
    if qrs_score >= 8:
    ...
    output_file = output_dir / f"{symbol}_{entry_point['entry_id']}_ultra_clean.png"
...
def create_ultra_clean_summary_visualization(entry_points_df: pd.DataFrame, output_dir: Path):
    """Create ultra clean professional summary visualizations."""
    print("📊 Creating Ultra Clean Professional Summary Visualizations...")
    ...
    fig.suptitle('Zone Fade Entry Points - Ultra Clean Analysis', fontsize=20, fontweight='bold', y=0.98)
    
    # Ultra clean color scheme
    colors = ['#3498db', '#e74c3c', '#2ecc71', '#f39c12', '#9b59b6', '#1abc9c']
    
    # QRS Score Distribution - NO FILL
    ax1 = axes[0, 0]
    ...
    # Entry Points by Symbol - clean pie chart
    ax2 = axes[0, 1]
    ...
    # Window Duration Distribution - NO FILL
    ax3 = axes[0, 2]
    ...
    # QRS Score vs Window Duration - clean scatter
    ax4 = axes[1, 0]
    ...
    # Zone Strength Distribution - NO FILL
    ax5 = axes[1, 1]
    ...
    # Monthly Distribution - clean bars
    ax6 = axes[1, 2]
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "ultra_clean_professional_summary.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    print("   ✅ Ultra clean professional summary visualizations created")


def main():
    """Main visualization function."""
    print("🎨 Ultra Clean Zone Fade Entry Points Visualization Tool")
    print("=" * 70)
    
    # Create output directory
    output_dir = Path("results/visualizations_ultra_clean")
    ...
    for idx, entry_point in entry_points_df.iterrows():
        ...
        print(f"   📊 Creating ultra clean visualization for {entry_id} ({symbol})...")
        
        if symbol in symbols_data:
            success = create_ultra_clean_visualization(
                entry_point, 
                symbols_data[symbol], 
                symbol, 
                output_dir
            ...
            if success:
                ...
                print(f"     ✅ Created ultra clean visualization for {entry_id}")
            else:
                failed_visualizations += 1
                print(f"     ❌ Failed to create ultra clean visualization for {entry_id}")
        else:
            print(f"     ❌ No data available for {symbol}")
            failed_visualizations += 1
    
    # Create ultra clean professional summary visualizations
    create_ultra_clean_summary_visualization(entry_points_df, output_dir)
    
    # Print results
    print(f"\n📊 Ultra Clean Visualization Results:")
    print(f"   ✅ Successful: {successful_visualizations}")
    print(f"   ❌ Failed: {failed_visualizations}")
    print(f"   📁 Output Directory: {output_dir.absolute()}")
    
    # Create an ultra clean index file
    index_file = output_dir / "README.md"
    with open(index_file, 'w') as f:
        f.write("# Ultra Clean Zone Fade Entry Points Visualizations\n\n")
        f.write(f"Generated ultra clean visualizations for {successful_visualizations} entry points.\n\n")
        f.write("## Key Features:\n")
        f.write("- **NO FILLED RECTANGLES**: Completely removed all filled areas and background clutter\n")
        f.write("- **CLEAN LINES ONLY**: All elements use clean lines and markers\n")
        f.write("- **NO OVERLAPPING LABELS**: Properly positioned information panels\n")
        f.write("- **ULTRA CLEAN DESIGN**: Minimalist, professional appearance\n")
        f.write("- **PRECISE TIME WINDOW**: Exactly 1 hour before + 2 hours after entry\n")
        f.write("- **CLEAN VOLUME**: Volume shown as clean vertical lines, not filled bars\n")
        f.write("- **HIGH QUALITY**: 300 DPI, publication-ready charts\n\n")
        f.write("## Files:\n")
        f.write("- Individual entry point visualizations: `{symbol}_{entry_id}_ultra_clean.png`\n")
        f.write("- Summary visualizations: `ultra_clean_professional_summary.png`\n\n")
        f.write("## Visualization Details:\n")
        f.write("- Time window: 60 minutes before entry + 120 minutes after entry\n")
        f.write("- Shows: Clean price action, Volume lines, VWAP line, Zone level\n")
        f.write("- Setup metrics: QRS score, rejection candles, volume spikes\n")
        f.write("- Ultra clean quality: Ready for professional analysis\n")
    
    print(f"\n🎉 Ultra clean visualization complete! Check {output_dir.absolute()}")
