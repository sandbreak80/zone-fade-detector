#!/usr/bin/env python3
"""
Simple Backtesting for Zone Fade Strategy - 2024 Data.
...
async def run_architecture_backtest(symbols_data: Dict[str, List[OHLCVBar]]):
    """Run backtesting with all architecture components."""
    print("\n🚀 Starting Architecture Backtest")
    print("=" * 60)
    ...
    for symbol, bars in symbols_data.items():
        ...
        results = await process_symbol_bars(
            ...
            micro_analyzer, cross_symbol_processor
        )
    ...
    generate_comprehensive_report(all_results, cross_symbol_processor, window_manager, session_manager, micro_analyzer)
...
async def main():
    ...
    print("🚀 Zone Fade Detector - Architecture Backtesting")
    print("=" * 60)
    ...
    results = await run_architecture_backtest(symbols_data)
    end_time = time.time()
    
    print(f"\n⏱️  Backtesting completed in {end_time - start_time:.2f} seconds")
    print("🎉 Architecture backtesting finished successfully!")
