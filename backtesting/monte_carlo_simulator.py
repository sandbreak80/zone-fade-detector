#!/usr/bin/env python3
"""
Monte Carlo Simulation for Zone Fade Strategy

This module runs 100 simulation runs to test if our results are realistic
or if there are calculation errors.
"""
...
class MonteCarloSimulator:
    ...
    def __init__(self, 
        ...
        # Realistic trading parameters (with more variation)
    ...
    def calculate_position_size(self, entry_price: float, stop_price: float, 
        ...
        """Calculate position size based on equity per trade."""
    ...
    def simulate_trade(self, entry: pd.Series, current_balance: float, 
        ...
        """Simulate a single trade with random exit scenario."""
        ...
        # Determine exit scenario (with some randomness)
    ...
    def _determine_exit_scenario(self, entry: pd.Series, simulation_id: int) -> str:
        """Determine exit scenario with some randomness per simulation."""
        # Use simulation_id as seed for consistency within simulation
    ...
    def print_monte_carlo_summary(self, stats: Dict[str, any]):
        ...
        print("\n" + "="*120)
        print("MONTE CARLO SIMULATION RESULTS - ZONE FADE STRATEGY")
        print("="*120)
...
def run_monte_carlo_analysis(csv_file_path: str, 
                           initial_balance: float = 10000.0,
                           max_equity_per_trade: float = 0.10,
                           slippage_ticks: int = 2,
                           commission_per_trade: float = 5.0,
...
if __name__ == "__main__":
    ...
    results = run_monte_carlo_analysis(csv_file, num_simulations=100)