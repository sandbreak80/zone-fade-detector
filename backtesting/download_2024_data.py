#!/usr/bin/env python3
"""
Download 2024 historical data for Zone Fade Detector testing.
This script downloads and caches data for the entire year 2024.
"""
...
async def download_2024_data():
    """Download and cache 2024 data for all symbols."""
    ...
    symbols = ['SPY', 'QQQ', 'IWM']
    start_date = datetime(2024, 1, 1, 9, 30)  # Start of 2024
    ...
    logger.info(f"📅 Downloading 2024 data from {start_date} to {end_date}")
    logger.info(f"📊 Symbols: {', '.join(symbols)}")
    logger.info(f"💾 Saving to: {data_dir}")
    
    # Send initial status to Discord
    await send_discord_status(f"🚀 Starting 2024 data download for {', '.join(symbols)}", alert_system)
    ...
    for i, symbol in enumerate(symbols, 1):
        ...
        try:
            ...
            if bars:
                ...
                symbol_file = data_dir / f"{symbol}_2024.pkl"
    ...
    if all_data:
        ...
        await send_discord_status("❌ Download failed - no data retrieved", alert_system)

if __name__ == "__main__":
    asyncio.run(download_2024_data())
