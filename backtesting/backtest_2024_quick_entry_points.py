#!/usr/bin/env python3
"""
Quick Zone Fade Entry Points Backtesting - 2024 Data.

This script runs a quick test on a subset of data to find entry points:
- Zone Detection
...
def load_2024_data_sample():
    """Load a sample of 2024 data for quick testing."""
    print("📊 Loading 2024 Data Sample...")
    ...
    for symbol in ["SPY", "QQQ", "IWM"]:
        ...
        if file_path.exists():
            ...
            with open(file_path, 'rb') as f:
                data = pickle.load(f)
                # Take only the last 10,000 bars for quick testing
                symbols_data[symbol] = data[-10000:] if len(data) > 10000 else data
...
def calculate_simple_qrs_score(bar: OHLCVBar, zone: Zone, bars: List[OHLCVBar]) -> float:
    """Calculate a simple QRS score."""
    score = 0.0
...
def run_quick_entry_points_backtest(symbols_data: Dict[str, List[OHLCVBar]]):
    """Run quick Zone Fade strategy to find entry points."""
    print("\n🚀 Starting Quick Zone Fade Entry Points Backtest")
    print("=" * 60)
    ...
    for symbol, bars in symbols_data.items():
        ...
        # Process bars for entry points
        results = process_symbol_for_quick_entry_points(symbol, bars, zones)
        
        all_results[symbol] = results
        print(f"   ✅ {symbol} processing complete")
    
    # Generate comprehensive report
    generate_quick_entry_points_report(all_results)
    
    return all_results


def process_symbol_for_quick_entry_points(symbol: str, bars: List[OHLCVBar], zones: List[Zone]):
        ...
        for zone in zones:
            if is_zone_touched(bar, zone):
                ...
                if is_rejection_candle(bar, zone):
                    ...
                    # Calculate QRS score
                    qrs_score = calculate_simple_qrs_score(bar, zone, historical_bars)
...
def generate_quick_entry_points_report(all_results: Dict[str, Any]):
    """Generate comprehensive entry points report."""
    print("\n📊 QUICK ZONE FADE ENTRY POINTS REPORT")
    print("=" * 60)
    ...
    print(f"\n🎉 Quick Zone Fade Entry Points Backtesting Complete!")


def main():
    """Main backtesting function."""
    print("🚀 Zone Fade Detector - Quick Entry Points Backtesting")
    print("=" * 60)
    
    # Load data
    symbols_data = load_2024_data_sample()
    ...
    results = run_quick_entry_points_backtest(symbols_data)
    end_time = time.time()
    
    print(f"\n⏱️  Backtesting completed in {end_time - start_time:.2f} seconds")
    print("🎉 Quick entry points backtesting finished successfully!")
