#!/usr/bin/env python3
"""
Parallel Zone Fade Entry Points Backtesting - 2024 Data.

This script runs Zone Fade strategy in parallel using 3 threads (one per asset):
- SPY, QQQ, IWM processed simultaneously
- Zone Detection, Rejection Candle Detection, QRS Scoring
- Full 2024 dataset processing
"""
...
def process_symbol_parallel(symbol: str, bars: List[OHLCVBar], progress_lock: Lock, progress_counter: Dict[str, int]) -> Dict[str, Any]:
...
def run_parallel_entry_points_backtest(symbols_data: Dict[str, List[OHLCVBar]]):
    """Run Zone Fade strategy in parallel using 3 threads."""
    print("\n🚀 Starting Parallel Zone Fade Entry Points Backtest")
    print("=" * 60)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all tasks
        future_to_symbol = {
            executor.submit(process_symbol_parallel, symbol, bars, progress_lock, progress_counter): symbol
    ...
    generate_parallel_entry_points_report(all_results)
    
    return all_results


def generate_parallel_entry_points_report(all_results: Dict[str, Any]):
    """Generate comprehensive entry points report."""
    print("\n📊 PARALLEL ZONE FADE ENTRY POINTS REPORT")
    print("=" * 60)
    ...
    print(f"\n🎉 Parallel Zone Fade Entry Points Backtesting Complete!")


def main():
    """Main backtesting function."""
    print("🚀 Zone Fade Detector - Parallel Entry Points Backtesting")
    print("=" * 60)
    ...
    results = run_parallel_entry_points_backtest(symbols_data)
    end_time = time.time()
    
    print(f"\n⏱️  Backtesting completed in {end_time - start_time:.2f} seconds")
    print("🎉 Parallel entry points backtesting finished successfully!")
