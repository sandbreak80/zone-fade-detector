...
Uses proper candlestick rendering with matplotlib.
"""
...
class ProperCandlestickVisualizer:
    """Proper candlestick entry point visualizer."""
    ...
    def create_entry_visualization(
        ...
        """Create a proper candlestick entry point visualization."""
        ...
        # Create chart
        fig = self._create_proper_candlestick_chart(
            chart_df, 
            symbol,
            entry_id,
            entry_timestamp
        ...
        # Save chart
        filename = f"{symbol}_{entry_timestamp.strftime('%Y%m%d_%H%M')}_candlesticks.png"
    ...
    def _create_proper_candlestick_chart(self, df: pd.DataFrame, symbol: str, entry_id: str, entry_timestamp: datetime) -> plt.Figure:
        """Create the proper candlestick chart."""
        ...
        self._plot_proper_candlesticks(ax_price, df)
    ...
    def _plot_proper_candlesticks(self, ax, df: pd.DataFrame):
        ...
        for i, (timestamp, row) in enumerate(zip(timestamps, df.itertuples())):
            ...
            bar_width = 0.3 / 24  # 0.3 hours converted to days
            ...
            if body_height > 0:
                ...
                ax.plot([timestamp - bar_width/2, timestamp + bar_width/2], 
                       [open_price, open_price], color=edge_color, linewidth=1.5)
    
    def _plot_vwap_line(self, ax, df: pd.DataFrame):
        """Plot VWAP line."""
        vwap_color = '#ff9800'  # Orange
        timestamps = mdates.date2num(df['timestamp'])
        ax.plot(timestamps, df['vwap'], color=vwap_color, linewidth=2, alpha=0.8)
    ...
    def _plot_volume_bars(self, ax, df: pd.DataFrame):
        ...
        for i, (timestamp, row) in enumerate(zip(timestamps, df.itertuples())):
            ...
            # Plot volume bar
            ax.bar(timestamp, volume, width=bar_width, color=color, alpha=volume_alpha, 
    ...
    def generate_entry_visuals(
        ...
        """Generate proper candlestick visualizations for all entry points."""
        print(f"\n🎯 Generating proper candlestick visualizations for {len(entry_points_df)} entry points...")
        print("   ONLY: Standard OHLC candlesticks + VWAP line + Volume bars")
        ...
        for idx, entry_point in entry_points_df.iterrows():
            ...
            print(f"   📊 Creating proper candlestick visualization for {entry_id} ({symbol})...")
            ...
            if symbol in symbols_data:
                ...
                if success:
                    ...
                    print(f"     ✅ Created proper candlestick visualization for {entry_id}")
                else:
                    results['failed'] += 1
                    print(f"     ❌ Failed to create proper candlestick visualization for {entry_id}")
            else:
                results['no_data'] += 1
...
def main():
    """Main function to run the proper candlestick visualizer."""
    print("📊 Proper Candlestick Entry Point Visualizer")
    print("=" * 50)
    print("ONLY: Standard OHLC candlesticks + VWAP line + Volume bars")
    ...
    output_dir = Path("outputs/visuals_candlesticks")
    ...
    try:
        ...
        # Print results
        print(f"\n📊 Proper Candlestick Visualization Results:")
        print(f"   ✅ Successful: {results['successful']}")
        print(f"   ❌ Failed: {results['failed']}")
        print(f"   ❌ No Data: {results['no_data']}")
        print(f"   📁 Output Directory: {output_dir.absolute()}")
        
        print(f"\n🎉 Proper candlestick visualization complete! Check {output_dir.absolute()}")
